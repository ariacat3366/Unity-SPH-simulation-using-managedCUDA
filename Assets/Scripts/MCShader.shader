		...
		Pass
		{
			...
			CGPROGRAM
				...
				GS_INPUT VS_Main(uint id : SV_VertexID)
				{
					...
					output.pos = float4( float3(id%_GS, (id / _GS) % _GS, (id / _GS / _GS)) * _GL , 1);
				...
				float4 MCDensity(int id) {
					if (0 <= id && id < M) return MCBuf[id];
				...
				void GS_Main(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream)	{
					...
					const float halfSize = 0.5 * _GL;
	 				const float4 cubeVerts[8] = {
						//front face
					float4(0, 0, 0, 0) ,			//LB   0
					float4(0,  _GL, 0,	0) ,		//LT   1
					float4(_GL,  _GL, 0, 0) ,		//RT   2
					float4(_GL, 0, 0, 0) ,			//RB   3
						//back
					float4(0, 0,  _GL, 0),			// LB  4
					float4(0,  _GL,  _GL, 0),		// LT  5
					float4(_GL,  _GL,  _GL, 0),		// RT  6
					float4(_GL, 0,  _GL, 0)			// RB  7
					};
					const float4 samples[8] = {
						MCDensity(id),
						MCDensity(id + _GS),
						MCDensity(id + _GS + 1),
						MCDensity(id + 1),
						MCDensity(id + _GS * _GS),
						MCDensity(id + _GS * _GS + _GS),
						MCDensity(id + _GS * _GS + _GS + 1),
						MCDensity(id + _GS * _GS + 1)
					};
					...
					for( int i = 0; i < 5; i++ ){
						if(i >= numpolys){
							...
							*/
							...
							float3 normalA = lerp(normalWeights[va], normalWeights[vb], amount);
							...
							float3 normalB = lerp(normalWeights[va], normalWeights[vb], amount);
							...
							float3 normalC = lerp(normalWeights[va], normalWeights[vb], amount);
							...
							pIn.tex0 = float2(1.0f, 0.0f);
							pIn.normal = normalA;
							pIn.viewDir = mul(unity_ObjectToWorld, worldPosA).xyz - _WorldSpaceCameraPos;
							triStream.Append(pIn);
							...
							pIn.tex0 = float2(1.0f, 0.0f);
							pIn.normal = normalB;
							pIn.viewDir = mul(unity_ObjectToWorld, worldPosB).xyz - _WorldSpaceCameraPos;
							triStream.Append(pIn);

				
							pIn.pos = pC;
							pIn.worldpos = worldPosC;
							pIn.tex0 = float2(1.0f, 0.0f);pIn.normal = normalC;
							pIn.viewDir = mul(unity_ObjectToWorld, worldPosC).xyz - _WorldSpaceCameraPos;
							triStream.Append(pIn);
				...
				fixed4 FS_Main(FS_INPUT input) : COLOR
				{
					...
					//_RenderPaintTexture
					//if (length(input.normal) < 1) { discard; }
					//float col = 0.3 + 0.7 *dot(normalize(input.normal) , normalize(float3(1,1,1)));
					//return fixed4(fixed3(1,1,1)*col , 1);
					//return _SpriteTex.Sample(sampler_SpriteTex, input.tex0)  * saturate(0.5 + input.normal.y * 0.5) ;

					float p = 3;
					float3 vec_lec = 2 * dot(-viewDir, normal)*normal + viewDir;//hansya
					float3 vec_rac = -1 / p*(sqrt(p*p - 1 + pow(dot(viewDir, normal), 2)) + dot(viewDir, normal))*normal + viewDir / p;

					float lec_v = lerp(1, pow(dot(viewDir, -normal), 5), 0.2);
					fixed4 col = lerp(texCUBE(_Cube, vec_lec), texCUBE(_Cube, vec_rac), lec_v);
